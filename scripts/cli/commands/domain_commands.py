...
def create_domain_structure(base_path: Path, domain_name: str):
    ...
    folders = ["entities", "repositories", "use_cases", "value_objects"]
...
@app.command("create")
def create_domains(
    names: List[str] = typer.Argument(..., help="List of domain names to create")
):
    if not names:
    ...
    typer.echo(
        f"Successfully created domain{'s' if not is_single else ''}: {', '.join(names)}"
    )
    return True
